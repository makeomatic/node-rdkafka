...
var Emitter = require('node:events').EventEmitter;
...
Client.prototype.disconnect = function(cb) {
  ...
  if (!this._isDisconnecting && this._client) {
    ...
    // NOTE: unless this is scheduled for a next tick - we can be in a loop on c++ side
    // where queue of async callbacks is invoked multiple times. there are no checks that
    // callback has not been deregistered during processing of the loop
    // and it will segfault if .disconnect() is called because it will set consumeLoop to NULL
    // and callback will become a null ptr, therefore causing the crash
    // scheduling this at the end of the current loop will allow sync c++ loop to finish furst
    setImmediate(() => {
  ...
  } else if (this._isDisconnecting && cb) {
    self.once('disconnected', (metrics) => cb(null, metrics));
  } else if (!this._client && cb) {
